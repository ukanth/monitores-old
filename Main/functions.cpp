...
extern "C" __declspec(dllimport) int GetStatusM();
...
LRESULT CALLBACK GetKeyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    if(GLOBAL_HOOK) CallNextHookEx(NULL, nCode, wParam, lParam);
    if (nCode == HC_ACTION && GLOBAL_HOOK == FALSE)
         SendMessage(global_hwnd, WM_SYSCOMMAND,SC_MONITORPOWER, (LPARAM)2);
     return CallNextHookEx(keyHook, nCode, wParam, lParam);

}


LRESULT CALLBACK GetMouseHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    if(GLOBAL_HOOK) CallNextHookEx(NULL, nCode, wParam, lParam);
       if (nCode == HC_ACTION && GLOBAL_HOOK == FALSE)
            SendMessage(global_hwnd, WM_SYSCOMMAND,SC_MONITORPOWER, (LPARAM)2);
...
{
    global_hwnd = hwnd;
    keyHook = SetWindowsHookEx(WH_KEYBOARD_LL, GetKeyboardHookProc, NULL, 0);
    mouseHook = SetWindowsHookEx(WH_MOUSE_LL, GetMouseHookProc, NULL, 0);
    GLOBAL_HOOK =  FALSE;
    Sleep(500); // Eliminate user's interaction for 500
...
void MuteVista(bool flagMute,int flag)
{
    typedef void (WINAPI*cfunc)();
    cfunc MuteAudio,UnMuteAudio,GetStatusM;
    char *mod = new char[256];
    HINSTANCE hLib = ::LoadLibrary("MuteVista.dll");
    GetModuleFileName((HMODULE)hLib, (LPTSTR)mod, MAXMODULE);
    if (hLib != NULL)
    {
        ...
        GetStatusM = (cfunc)GetProcAddress((HMODULE)hLib, "GetStatusM");

        //int returnValue = GetStatusM();
        //if(returnValue) alert("YES","coming");
        //else alert("NO","Coming");
...
void MuteMasterVolume(bool flagMute,int flag)
{
    if (IsWindowsVista())
    {
        MuteVista(flagMute,flag);
    }
    else
    {
...
static char *
sGetItemText (HWND hWnd, int nItem, int nSubItem)
{
    LVITEM	lvi;
    DWORD	TID,
    PID;
    HANDLE	hRemoteProcessHandle;
    PSTR	pszRemoteProcessMemory;
    int		nLen = 128;
    int		nRes;
    char	*p3;
    DWORD	dwRead;
    static TCHAR szBuffer[_SIZE] = {0, };
    ...
    pszRemoteProcessMemory = (PSTR) VirtualAllocEx (hRemoteProcessHandle, 0, _SIZE, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    ...
    p3 = (char*) pszRemoteProcessMemory + sizeof (LVITEM);
    do
    {
        nLen *= 2;
        ...
        nRes = (int) SendMessage (hWnd, LVM_GETITEMTEXT, (WPARAM) nItem, (LPARAM) pszRemoteProcessMemory);
    }
    while (nRes == nLen - 1);
...
void ControlWMP()
{
    ...
    HWND hwnd = FindWindow ( "WMPlayerApp", NULL );
    //alert("hello","");
    if ( hwnd != NULL )
    {
        //ugly code
        hwnd	= FindWindowEx (hwnd, 0, "WMPAppHost", NULL);
        hwnd	= FindWindowEx (hwnd, 0, "WMP Skin Host", NULL);
        hwnd	= FindWindowEx (hwnd, 0, "CWmpControlCntr", NULL);
        hwnd	= FindWindowEx (hwnd, 0, "WMPPlaylist", NULL);
        //hwnd	= FindWindowEx (hwnd, 0, "ATL:SysListView32", NULL);
        //alert("","entering");
            UINT i = 0,
            count	= 0;
            LRESULT	lr;
            ...
            for (i = 0; i < count; i++)
            {
                lr	= SendMessage (hwnd, LVM_GETITEMSTATE, i,
                                  LVIS_CUT | LVIS_DROPHILITED | LVIS_FOCUSED | LVIS_SELECTED | LVIS_OVERLAYMASK | LVIS_STATEIMAGEMASK);

                if ( (lr & LVIS_FOCUSED) == LVIS_FOCUSED)
                {
                    char *psz = sGetItemText (hwnd, i, 0);
                    // probably only has the track name available...
                    // check for '-' and assume <artist> - <title>...
               //     alert("",psz);

                }
            }
            //return (pTD->status = kPlayerStatus_Playing);
        //}
                SendMessage ( hwnd, WM_COMMAND, 0x00004978, 1 ); // Pause/play
    }
