    ...
    while ((gRet = GetMessage (&messages, NULL, 0, 0)) != 0 )
    {
        ...
        else
        {
            ...
            if (messages.message == WM_HOTKEY)
            {
                ...
                if (messages.lParam == SCR_LPARAM || HIWORD ( messages.lParam ) == 0x1B )
                {
                    //TurnOnScreenSaver(hwnd);
                    SendMessage(global_hwnd, WM_SYSCOMMAND,SC_MONITORPOWER, (LPARAM)-1);
                    GLOBAL_HOOK = TRUE;
    ...
    case WM_WTSSESSION_CHANGE:
        ...
        case WTS_SESSION_LOCK:
            if(GLOBAL_OP_FLAG == 1)
            {
                ...
                if (IDC_MUTE_VOL_FLAG) MuteMasterVolume(true,0);
                ControlMessengers();
                GLOBAL_OP_FLAG = 0;
                break;
            }
        case WTS_SESSION_UNLOCK:
            if(GLOBAL_OP_FLAG == 1)
            {
                ...
                if (IDC_MUTE_VOL_FLAG) MuteMasterVolume(false,1);
                RestoreMessengers();
                GLOBAL_OP_FLAG = 0;
